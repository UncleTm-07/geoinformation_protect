...
const ServerWidget = ({widget, setWidget, servers, addNewServer, getCoordinateForFlyTo}) => {
                ...
                <div className={"widget_list_container"}>
                    {
                        _servers?.map((value, index) => (
                            <ServerCard key={`Server-ID-${value.id}`} value={value} index={index} getCoordinate={getCoordinate}/>
                        ))
                    }
                </div>
            </div>
